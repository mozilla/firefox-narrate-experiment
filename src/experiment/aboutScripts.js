{
  // This is relaxed version of content-scripts API to allow matching about:* pages
  // based off the native implementation, specifically of the following revision
  // https://github.com/mozilla/gecko-dev/blob/2ee9341d0149cc772c6d8a5a010733faa05c1c87/toolkit/components/extensions/parent/ext-contentScripts.js
  ...
  this.aboutScripts = class extends ExtensionAPI {
    getAPI(context) {
      ...
      return {
        aboutScripts: {
          async register(details) {
            // details.matches = details.matches.map(
            //   ({ pattern, restrictSchemes, ignorePath }) =>
            //     new MatchPattern(pattern, { restrictSchemes, ignorePath })
            // )
          ...
          async unregister(scriptId) {
            ...
            if (!contentScript) {
              Cu.reportError(
                new Error(`No such content script ID: ${scriptId}`)
              )
